...
void Odometry::update() {
    ...
    int delta_parallel = this->parallel == nullptr ? 0 : this->parallel->getCurrentPosition() - this->last_parallel;
    ...
    double delta_x = ((delta_right + delta_left) / 2.0);
    double delta_y = delta_parallel - (phi * this->perp_offset);
...
void Odometry::reset_deltas() {
    ...
    this->last_parallel = this->parallel == nullptr ? 0 : this->parallel->getCurrentPosition();
